...
public class DAOEventToday extends EventNext implements DAO{
    ...
    public List<EventNext> getEventNext (String kdTraveller){
        ...
        try{
            while(rs.next()){
                EventNext td = new EventNext();
                td.setKdEvent(rs.getString("Kd_Event"));
                td.setKdTraveller (rs.getString("Kd_Traveller"));
                td.setTravelMode(rs.getString("Travel_Mode"));
                td.setAvoidtolls(rs.getInt("Avoidtolls"));
                td.setNamaEvent(rs.getString("Nama_Event"));
                td.setWaktuMulai(new Date(rs.getTimestamp("Waktu_Mulai").getTime()));
                td.setWaktuSelesai(new Date(rs.getTimestamp("Waktu_Selesai").getTime()));
                td.setKeterangan(rs.getString("Keterangan_Event"));
                listEvent.add(td);
            }            
        }catch (SQLException ex){
            System.out.println("terjadi kesalahan saat menampilkan EventToday : ");
            ex.printStackTrace();
        }
        return listEvent;
    }
